...
// MCPServerConfig turns MCP agent access on or off for the instance. Declared here because
// the generator only copies structs from the instance's own proto package; the API type lives
// in types/mcp/v1.
type MCPServerConfig struct {
	// No omitempty: an exported manifest must carry enabled: false for apply (a merge patch) to turn MCP off.
